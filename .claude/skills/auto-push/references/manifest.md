# Change 清单数据目录（docs/auto-push/changes/）

`docs/auto-push/changes/<change-id>.json` —— 一个 Change 一个清单，由
`scripts/autopush.py` 创建与维护（TASK-101 / ADR-0079）。人读任务卡
（`docs/tasks/TASK-*.md`），机器读这里；两者经 commit hash 互相印证。
