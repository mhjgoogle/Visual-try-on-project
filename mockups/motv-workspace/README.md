...
**Production Studio 视觉收敛（TASK-051，进行中）**：制作视图已按
[TASK-051](../../docs/tasks/TASK-051-production-studio-ui-convergence.md) 重建为
专业制片工作室外壳——顶栏 `[项目 ▾] [制作] [工作流] [资产]`；左栏 = 项目（故事/
作品设定/剧集）+ **剧集选择器 EP01…EPnn** + 本集六阶段（带进度条）；中栏 = 媒体优先
工作区；右栏 = 常驻 AI 导演（当前上下文 → 导演建议 → 指令 → **Prompt 预览** →
生成方式 → 一个主行动 → 真实生成记录）。**已重建**：故事 / 作品设定 / 分镜 / 画面 /
视频。**尚未重建**（保持既有观感，功能不变）：音频 / 剪辑 / 工作流生产地图 / 资产库。
数据与行为完全不变：schema 仍是 v9，无迁移，无新 Provider，UI 状态全部 transient。
**演示种子**：演示模式打开示例项目会经领域 API 生成一个「做到一半」的项目
（`fixtures/demo-project.js`），媒体是运行时 `data:` SVG 占位图
（`fixtures/demo-media.js`，每张都印着「占位素材 · PLACEHOLDER」，不提交任何二进制）；
连接模式永不触发种子。下文的旧 IA 描述保留为历史记录。
