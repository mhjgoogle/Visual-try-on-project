...
| `completed` | **派生 + 要证据**：产物存在，且探针**没有否认也没有说不知道** | 唯一能防漂移的做法 |
| `skipped` | **存储** | 只有人 / Workflow 的决定能知道 |

**只有 `skipped` 是真正新增的持久状态。** 其余三个是把已有证据读成一个词。

#### 订正（2026-08-18，TASK-097 批次 3 codex 轮 5 的 blocking finding）

上面这一行原本写的是「探针**没有判定它 MISSING**」。那句话**太松**，而且实现照它写了：
`INCONCLUSIVE`（我们问过，答不上来）不是 `MISSING`，于是它被算成产物存在，
**闸门会在未经确认的媒体上打开**。

探针有四种答案，它们**不能塌缩成两种**。同一个三态被两个不同的问题读，答案不同：

| 问题 | `PRESENT` | `MISSING` | `INCONCLUSIVE` | 从未问过 |
| --- | --- | --- | --- | --- |
| 「这张图丢了吗」（`mediaprobe.js` 的规则） | 没丢 | **丢了** | 不说丢了 | 不说丢了 |
| 「这一步做完了吗」（**本决策**） | 做完了 | 没做完 | **不说做完了** | 做完了 |

两条都对，因为它们防的是**相反方向**的误报：前者不许探针成为「你的文件不见了」
这句话的来源，后者不许「答不上来」变成「已完成」。

「从未问过」与 `INCONCLUSIVE` 是**两种不同的处境**，不可合并：前者是没人问，
此时登记表的 current 指针就是我们手上的证据（探针是懒扫描的）；
后者是问过而且答案是「说不清」——那是一条真实的怀疑证据。
