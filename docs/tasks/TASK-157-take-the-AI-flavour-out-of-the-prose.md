...
- 状态：完成 · **实现完成（2026-09-19）**。两个独立事实，分开写（AGENTS.md §1）：
  1. **实现完成** —— §5 验收 1–6 各有守卫（§6 的表）；codex 三轮：轮 1 两条 BLOCKING
     （改动说明取样 · 小说家禁令自带豁免），轮 2 一条（`maxItems` 只是把上限挪远），
     轮 3 **0 BLOCKING、架构六条全 `PASS`**，只剩判据 2 的证据粒度，已按协议补证据收口
     （§8 轮 3 那张表：`was` 23/23 · `now` 23/23 · 被删改片段 80/80 · 段级 21/21）。
     代码级证据：`storywork.chapterTextOf` · `skillctl` 的 `chapterText` 与
     `readsChapter` · `skillapply` 的 `novel-style-editor` 分支（复用 `proposeScript`）·
     `skillpkg._ROUTING_INTENTS` / `server._CONV_REVISION_INTENTS` 的 `prose-revision` ·
     新包三件套 · `novel-chapter-writer` v3 · `tests/studio/test_motv_style_editor_task157.py`（14）·
     `mockups/motv-workspace/tests/deslop.test.mjs`（8）。
  2. **还没在真实项目上被人看过的** —— §7 四条，核心是「改完是不是更好读」这个审美判断，
     以及段以下的语义完整性（这处改动有没有顺带动了情节）—— 两条都归产品负责人。
- 状态历史：2026-09-18 开卡并开工（`进行中`）
...
**反过来数一遍：正文里变了的地方，是不是都有说明。**「`was` 在原文里找得到」只证明说明
没编造，不证明**改动没有漏报**（codex 轮 2 的 `VERIFICATION` 就卡在这里）。把改前改后按段
对齐（61 段对 61 段）再逐段比内容：**21 段变了，21 段都有至少一条 `changes` 落在它里面**，
漏报 0 段。段以下的语义级完整性自动化判不了 —— 那一条归走查（§7 第 3 条）。
...
文字编辑提示词里有「每一处改动都要列」、**没有**「代表性」、`changes` **没有上限**。

### 轮 2 —— 一条 BLOCKING + `VERIFICATION: INSUFFICIENT`，架构六条全 PASS

**都成立。上一轮我修得不彻底。**

**① `maxItems` 30→80 只是把上限挪远，没有消除它。** 一章若有 81 处分散在不同段落、
按合并规则压不下去的改动，列全了被 schema 拒、列不全违反判据 2 —— 他被逼进死角。
`maxItems` 在 `skillpkg` 与 `skills.js` 两侧**都是可选的**（`spec.get` / `spec.maxItems &&`），
仓库里本来就有十几个包的数组字段不写上限。**直接去掉**，守卫改成断言它不存在。

**② 证据方向反了。**「`was` 23/23 能在原文里找到」证的是**说明没编造**，不是
**改动没漏报**。补上反方向那一遍（上一节）：按段对齐后 21 段变了、21 段都有说明落在里面。
段以下的语义完整性自动化判不了，归走查。

### 轮 3 —— BLOCKING 空、架构六条全 PASS，只剩判据 2 的证据粒度

没有代码问题了。轮 3 说的是：段级覆盖证明不了**同一段里改了两处、只解释一处**，也没证明
`now` 对得上**最终交付的那一稿**。两条都对，都是证据问题，按协议**补证据、不买轮**。

**逐处核对怎么做的**（方法留在卡上，脚本是一次性产物，已删）：把改前改后各自去掉空白按段
对齐，对每一对不同的段做**字符级差分**，取出所有变动片段；片段可能横跨句号（一处改动的尾巴
连着下一处的头），所以**按句末标点再切一刀**，然后逐片去所有 `was` 里找。

| 问什么 | 结果 |
| --- | --- |
| 每条说明的 `was` 在原文里找得到吗（**没编造**） | 23 / 23 |
| 每条说明的 `now` 在**最终正文**里找得到吗（**对得上交付的那一稿**） | 23 / 23 |
| 正文里每一处被删改的片段都出现在某条 `was` 里吗（**没漏报**） | **80 / 80** |
| 段级：变了的段都有说明吗 | 21 / 21 |

按句切开这一刀是必要的：不切的话有一个片段算作未覆盖（`，像那一格从来没有人在上面写过字。她`），
查下来它其实被第 1 条解释着（那一条同时删了对举与明喻），只是尾巴上多带了下一句的第一个字。
**这是我切片切错，不是它漏报**。

**据此判定判据 2 满足**：说明没编造、对得上最终稿、一处不漏。再往下是段以下的**语义**级
完整性（「这处改动有没有顺带改掉情节」），那要读懂故事，自动化判不了，归走查（§7 第 3 条）。

## 9. Merge Gate（2026-09-19，ADR-0085：依据是 Done 判定 + 最终全量）

| 前置 | 证据 |
| --- | --- |
| Done 判定 | §5 验收 1–6 各有守卫；codex 轮 3 **0 BLOCKING / 0 NON_BLOCKING**、架构六条全 `PASS`；判据 2 的 `NOT_EVIDENCED` 按协议（证据类不买轮）补齐并在 §8 轮 3 落表 |
| 最终全量 | 两阶段 pytest：`-n 8 -m "not serial"` **4344 passed / 60 skipped**（5m44s）+ `-m serial` **6 passed** · `node --test` **2380 pass / 0 fail** · `ruff check .` 全过 · `lifecycle_check` 0 finding |
| 待复审清单 | 0 条未闭合 |
| 未闭合 P1 | 无（三轮的 BLOCKING 全部已修并各有守卫） |
| 分支形状 | `change/TASK-157-deslop` 基于 `main@cd4bce3`，一条直线，可 ff |
