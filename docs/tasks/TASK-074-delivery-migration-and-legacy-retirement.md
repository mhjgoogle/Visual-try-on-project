...
- 状态：待办 · **里程碑闸 2026-09-18 判「现在不做」**：§0.0 剩下的 B（`approveShot` 双写身份，
  需 ADR）、C（随 TASK-106 验收 3）、D（单一剧集制作主路）都不在当前里程碑 REQ-009 的交付面上、
  不阻塞它、不做无不可逆损害（三条都是一致性欠账，不是数据会变脏）。**什么条件下变成该做**：
  REQ-009 切片四「小说 → 剧集」把人带回剧集制作那条线时（D 会直接挡在路上）；或 TASK-106
  重新开工（C）；或 TASK-087 §4.12 的 ADR 被写出来（B）。
  **部分实施。** 下面「§0.0 当前剩余清单」是这张卡**唯一**的当前状态来源 ——
...
| 4 长任务刷新→从后端恢复 | ✅ **归属已改到驱动前端代码的测试上（2026-09-06，TASK-106 判据 5）**：`mockups/motv-workspace/tests/convresume.test.mjs`（28 条，真调 `resumePendingRun` → `landRun` 那条恢复链）＋ `tests/e2e/test_connected_journey_task130.py`（真浏览器：运行中刷新 → 页面从后端恢复同一 run → 放行后答案落进线程）。**下方 2026-08-25 的订正保留**，它记录的是这条为什么曾经退回 —— 那个 ⛔ 的理由（前端一处都不读运行状态）已由 `runState` 的调用点消除。旧的 `tests/studio/test_motv_runs_api_task072.py` 仍在，但它守的是后端 API 那一半，不再是这条边界的归属 |
...
**所以 §1.4 还剩什么**：**八条边界情况全部有自动守卫**（2026-09-06）。边界 4 曾于
2026-08-25 退回未闭合（上方订正：归属测试测的是后端 API，不是产品路径），
TASK-106 判据 5 把它的归属改到了两条**驱动前端代码**的测试上。
...
素材」，理由写在 [TASK-055 §5](TASK-055-project-rooted-storage.md)：占位
