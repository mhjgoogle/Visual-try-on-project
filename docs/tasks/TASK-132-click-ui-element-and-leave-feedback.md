...
- 状态：待办 · **里程碑闸 2026-09-18 判「现在不做」**：剩下的切片 C（截图）与 §5「还没做」
  两条不在当前里程碑 REQ-009 的交付面上、不阻塞它、不做无不可逆损害。已交付的 A / B 在
  main 上照常可用（REQ-006 判据 4/7/9 的反馈回路没有退）。**什么条件下变成该做**：产品
  负责人在真实项目上给意见时说「光有位置不够，要看到截图」；或 `data-ui-component` 打点
  那十几个渲染点有人顺路经过（那时把打点一起做）。
  **切片 A 完成、切片 B 完成核心、切片 C 未做**（2026-09-05 实施；证据见 §5「Claude 的实施切片」）。
  卡头原写「待开始；未实现点选功能」—— 那是 2026-09-06 发现的过期状态：切片 A/B 的提交
  （`597f426` / `8a9d57e`）早已落地，正文也一直写着完成，只有这一行没跟上。
  **一张卡的状态只能有一处**，以正文 §5 为准，这一行跟着它。
...
- `src/ui/production.js` 的 conversationContext 已携带 route、section、source、分集/镜头信息，
  MODULE_SOURCE 负责页面源码映射；[TASK-120](TASK-120-feedback-carries-a-locator.md)
