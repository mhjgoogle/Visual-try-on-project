...
- 状态：完成 · **实现完成**（2026-09-05 接手 · 2026-09-06 收口）。
  **代码级证据**：CI run `33982729462` / `33982727123`（`769904c`）两个 job 的
  每一步 success，其中一个跑的是**合并结果**；八条缺陷逐条修复见下文各节；
  触发面 `change/**` 已生效（同一提交跑出两个 run）。
  **还没有被人在真实项目上看过的**：无 —— 本卡的验收面就是 CI 本身，它是机器判定。
  **明确不做的**：把「CI 最近一次绿」并进 Merge Gate（本卡「要做什么」第 3 条是
  **或**关系，已由触发面满足；那一半要改 auto-push 脚本，属别人在办范围）。
...
| 红的那条 | 真正的原因 |
| --- | --- |
| `test_docs_links` | 守卫**没跳过围栏代码块**。`dev-workflow/references/lifecycle.md` 里有一段 ADR 双向取代的**模板**，写着 `[ADR-XXXX](done)`，目标字面量是三个点。Windows 会把纯点路径规范化掉（`x/...` → `x`，存在），Linux 不会。围栏里的东西根本不会被 markdown 渲染成链接，所以它不可能「断」—— 加了 `_strip_fences`（CommonMark 的同种字符 + 不短于开启的闭合规则），并留了 7 条形状用例证明它仍然抓得住真断链 |
...
接手时这一节写的是：「CI 会不会绿**我今天无法证明**」「**九天没人看见**这件事
本身没有被修」。两句现在都不成立了 —— 留着原话在这里，是因为它记录了当时
确实只有推断没有证据。

**第一半（会不会绿）**：不再是推断。开 PR 让 CI 跑**合并后的结果**，
`Merge … into cd9ef06` 两个 job 每一步 success。中间它还当场推翻过我自己的一句
推断（「簇 C 合并即消失」）—— 那条断链在合并结果上是红的。

**第二半（谁会被叫醒）**：`change/**` 进了触发面。此前日常工作全在这类分支上，
一条改动**第一次遇到 CI 是在它已经合进 main 之后** —— 唯一会喊的地方在最晚的
时刻，这才是「没人看见」的机制性原因，不是谁疏忽。

**这批缺陷的共同形状，比缺陷本身更值得留**：八条里有五条属于「本地永远绿」
（三条在本机会 skip，三条判定取决于操作系统），一条属于「从来没执行过」
（被前面的失败挡成 skipped）。**它们没有一条会被本地全量发现**，而这九天里
所有「测试全绿」的说法，依据的正是本地全量。

取证手段也留一条：**WSL 里的 Ubuntu-22.04 是 Python 3.10，与 CI 的 Ubuntu job
同一个小版本** —— 那些「只在 CI 上跑得到」的测试，在本机是可以真跑的。
（注意：在那棵 Windows 检出的树上跑 `tests/tooling` 全量会多出约 20 条红，
报 `set: pipefail: invalid option name`，那是 CRLF，不是缺陷。）
