...
- 状态：**实现完成**（2026-09-05；证据见 §实施记录）。**真浏览器已走过一遍**：[TASK-130](TASK-130-connected-sample-and-journey.md) 的旅程在可重复样本上打字进故事核心（`uiAct` → `work.core`），刷新后字还在；**真实用户项目**上的人工走查仍归产品负责人本人（收敛审查 §9）。ADR-0096 决策 4（派生页面地图 / 能力输入）不在本卡，另开卡
