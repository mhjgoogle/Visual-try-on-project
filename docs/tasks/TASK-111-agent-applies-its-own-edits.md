...
- 关联 Requirement：[REQ-004](../requirements/REQ-004-three-pane-shell-and-agent-conversation.md)
  **v2 判据「对话的另一端是能干活的 Agent」的后半段**（前半段「自己收集信息 + 回话」已由
  [TASK-109](TASK-109-three-pane-shell-and-agent-conversation.md) 交付）
...
- 「逐步可见」依赖 [TASK-106](TASK-106-frontend-run-path-and-legacy-endpoint-retirement.md)
...
- 画布文档被判为无效时，应用会加载空白画布并**封锁保存**（`persist._blocked`）。验证过程中
  ...
  [TASK-087 欠账总账](TASK-087-followup-ledger.md)。
