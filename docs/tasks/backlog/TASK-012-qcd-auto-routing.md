...
> **状态：Outline（WFM3）。**
>
> **2026-08-24 订正：前置其实全都满足了。** TASK-016 / 017 / 009 / 021 **四张
> 全在 `done/`**，ADR-0040 是 Accepted。原文把它们写成「依赖现行……」，读起来
> 像还没到位。
>
> 「路由权重须用户裁决」这句也要分两半看：**建这套机制不花一分钱**，按
> AGENTS.md §1「做一版给他看」，带着写明的默认权重实现出来即可；真正花钱的是
> **拿它去跑一次付费生成**，而那一步本来就被既有的付费闸挡着（ADR-0006 /
> 0009 的窄授权）。所以这里没有一个额外的、独立的「产品裁决闸」。
>
> **那为什么现在不做**：与 [TASK-011](TASK-011-local-video-provider.md) 同一个
> 理由 —— AGENTS.md §2「同一时间最多推进 1 个主要用户需求」。TASK-074 还开着。
> **这是排期，不是闸门。**
