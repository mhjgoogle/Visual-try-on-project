...
- 状态：完成 · **已完成（2026-08-24 收口）。** §5.4「本卡未做」六项里**五项 ✅**，
  ...
  **~~仍剩 `assets` / `actions` / `skills`~~ 这句 2026-08-23 复查为过期**：
  `assets` 与 `skills` 早已搬出（`controllers/assetctl.js` / `skillctl.js` 就在
  磁盘上），`actions` 是**按 §5.12 的收益标准判定不搬**并写明了理由。
  **注意：本卡是在前置 TASK-072 尚未
  全部验收的情况下开工的**（原状态行写「已规划，未开始」，与代码严重不符，
  2026-08-15 据 git log 与 §5 实施记录更正）
...
| 9 | 后端错误显示为错误，不显示为空 | ✅ **已守（2026-08-24）**：查询层 `tests/backend500.test.mjs`（三条读路径 500 时抛出而非回 `[]`，含「真的空仍是空」与「没有后端返回空合法」两条反方向对照）；渲染层 `tests/listerror.test.mjs`（钉住那条三轮审查才定下来的不变量：**只有 `fetchProjectList` 写 `LIST_ERROR`**，渲染只读）。变异验证把三轮的错法逐一放回去，全部转红 |
...
| §1.8 `app.js` 拆分 | 🟡 **持续项，不是一次性步骤**（见 5.16）—— 十个控制器已搬出，但 app.js 因后续里程碑的新功能又长回基线之上 |
...
**第三批搬出（2026-08-15，本会话接手 §1.8 剩余，产品负责人当日指示「拆」）**：

| 控制器 | 文件 | 依赖面 | 结果 |
| --- | --- | --- | --- |
| 资产 | `controllers/assetctl.js`（266 行） | 4 份文档、依赖清晰 | **已搬**，`tests/assetctl.test.mjs` 13 项 |
...
| --- | --- | --- | --- | --- | --- |
| `skills` | 853 | 10 | **0（全读）** | 12 | **值得搬**：读多写零，搬迁不会引入静默写错；是 `app.js` 里最大的一块。需要连续读完再一次性重写，**中途中断会留下「搬一半」，比不搬更糟** |
| `actions` | 470 | 5 | **1（`reviewsDoc`）** | **17** | **建议不搬**：它是所有写操作的分发中枢，按定义要够到每个域。搬出去后测它得 mock 整个世界，可测性收益接近零，而它同时是风险最高的一块（中枢 + 文档写入） |

对照已搬的四个：它们能写出真测试，是因为依赖能被显式列出来（`assetctl` 4 份文档、
`framectl` 3 份）。`actions` 的 17 个 `ctx` 依赖列出来也没有意义——那不是解耦，
只是把同样的耦合换了个文件放。**搬迁的收益标准是「搬完能不能写出真测试」，
不是「app.js 少了多少行」。**

### 一条被搬运「搬空」的守卫，以及正确的修法

`test_every_media_write_path_declares_at_the_write` 断言 `app.js` 里
`assetreg.checkDeclaration(` **至少 4 次**（四条上传路径各自预检）。`assetctl`
带走两处后它降到 2，测试失败。

**数字不下调**——那是放宽守卫掩盖问题。改的是扫描范围：现在扫
`app.js + src/controllers/*.js`，与 §5.12 给生成快照守卫做的处理相同。
自证有效：只扫 `app.js` 是 2（会失败），加上 controllers 是 5（通过）。
注释里写死了这个数字的含义是「四条上传路径各自预检」，不是「app.js 里还剩几处」，
免得下一次搬迁再把它调下去。

**每搬一个控制器都要检查这类守卫**：它们按文件扫描，而搬迁正好改变文件归属，
...
### 5.16 §1.8 复查：它是一条持续的线，不是一个做完就完的步骤（2026-08-23）

**先说复查结论，因为交接文档和本卡状态行都写错了。** 交接说「仍剩
`assets` / `actions` / `skills` 三个控制器」，本卡状态行也这么写 —— 都是过期的：

| 声称仍剩 | 实际 |
| --- | --- |
| `assets` | `controllers/assetctl.js`，266 行，13 项测试（§5.12 第三批） |
| `skills` | `controllers/skillctl.js`，957 行，26 项测试（§5.15 第四批） |
| `actions` | **按 §5.12 的收益标准判定不搬**，理由写在那里：17 个 `ctx` 依赖列出来也不是解耦，只是把同样的耦合换个文件放 |

**但真正该报的数字是另一个。** §5.15 说「§1.8 到此收口」时 app.js 是 5492 行；
今天是 **7803 行** —— 比 §1.8 开工时的基线 6992 还多 811 行。中间
TASK-094/095/096/097/098/103 各自往里加了 100～200 行（`git log` 逐提交可查）。

所以 §1.8 的验收 #14「本轮结束时 app.js 显著小于基线」**在它自己那一轮里成立过，
今天不成立**。这不是当时做错了，是**把一条持续的线当成了一次性步骤**：
每个新里程碑都往 app.js 加编排代码，而没有人负责把长出来的部分再抽走。

#### 第五批：提示词域（2026-08-23）

| 控制器 | 文件 | 为什么这三层一起搬 |
| --- | --- | --- |
| 提示词 | `controllers/promptctl.js` | `prompt` 是一份提示词的**版本账**；`basePrompt` 把一个基础资产实体翻译成那本账里的一个 key，它的 `save`/`setActive`/`useCompiled`/`setLocked` **四个写操作全部是对 `prompt` 的转发**；`promptBatch.confirm` 的成败判据来自这两层编译出来的结果。分开搬就是让「同一份提示词」的三层各持一段 |

`app.js` **7803 → 7583 行**（−220：搬走 271 行，接线加回 51 行）。

**这一批多了一样前四批没有的东西：一个会被整体替换的模块级 `let`。**
文档用 getter 注入就够（只读），但 `promptBatchState` 是**写侧也要**的 ——
控制器每一步都会替换它。给的是 `{ get, set }` 而不是把状态搬进控制器：
`restore` 换项目时仍然要能把它清掉，真相只留一份，在 app.js。

`_askGateway` 留在 app.js（它读 `ctx.gateway` 与 `query`，是这个文件负责组装的两样
东西），但仍然经 `ctx.promptBatch._askGateway` 这个属性调用 —— 那本来就是测试替换
的缝，直接调函数会让替换**悄悄失效**（替换的属性没人读）。

#### 又一次撞上「按文件扫描的守卫」，处理方式与 §5.12 相同

`tests/promptrefs.test.mjs` 里有六条守卫扫的是 `src/app.js` 的**源码文本**
（「确认要真的逐镜跑完」「await 回来不把报价贴错批次」「预检失败也落盘」……）。
搬走之后它们**全部转红** —— 这次是响的，比 §5.12 担心的「仍然通过但什么都没断言」
好，但根因一样。

处理：**不降数字、不删断言**，改两处 ——

1. 扫描面从 `app.js` 变成 `app.js + src/controllers/*.js`（`appSource()`），
   与 §5.12 给生成快照守卫、上传声明守卫做的处理一致；
2. 标识符跟着换（`promptBatchState` → `batchState.get()`，`PROJECT_NAME` →
   `projectName()`）—— 控制器**够不到** app.js 的模块级 `let`，getter 注入正是
   §5.10 定下的模式，所以新拼法才是正确的断言对象。

并给 `appSource()` 加了一条自检（扫描面 > 200 000 字符且两侧都命中），
因为扫描面为空的 scanner 会永远通过并且什么都不断言（TASK-087 §7 项 2）。

**变异验证**（守卫类改动一律做，交接文档第 3 条）：拆掉 `if (!stillMine()) return
null` → 「不把报价贴错批次」转红；把 `confirm` 的循环改成空数组 →
「真的逐镜跑完」转红；两次都撤回后全绿。

#### 剩下的怎么办

不再宣布「§1.8 收口」——那个词正是上一次让它失效的原因。**它是一条持续的线**：
`ctx` 字面量里现在还有 `story`(207) · `shotctx`(241) · `shot`(278) · `episode`(188) ·
`baseAssets`(254) · `breakdown`(173) · `audio`(153) 等域，各自都够格独立成控制器。
判据仍然是 §5.12 那一条 —— **搬完能不能写出真测试**，不是 app.js 少了多少行。
