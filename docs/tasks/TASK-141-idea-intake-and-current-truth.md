...
- 状态：完成 · **已完成**（2026-09-05 开卡 · 同日收口 · codex 独立审查 2 轮 pass）
...
- 存量卡不回填层级标注（OUT OF SCOPE），新想法按新入口走 —— 若三个月后仍有卡
  说不清自己在哪一层，再考虑回填。
- [总账 5.24](TASK-087-followup-ledger.md)：`gen_docs_status.py` 用定宽切片取 REQ 号，
  `REQ-1000` 会静默错配（P3，今天不可达）。
- [总账 5.25](TASK-087-followup-ledger.md)：AGENTS §14「一张卡一个 Agent」没有
