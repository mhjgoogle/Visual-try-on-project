...
| ② | `data/projects.json` 仍写在仓库内 | **中** | 这是**应用级**数据，既非源码也非项目数据，最终不应留在仓库。目标位置形如 `%LOCALAPPDATA%\motv\projects.json`，或用户指定的 `D:\MotvData\`。不阻塞使用，后续做一个很小的 App Storage checkpoint（[TASK-056](../active/TASK-056-app-storage-location.md)）。 |
