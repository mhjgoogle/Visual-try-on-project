...
> **状态：`done/` —— 产品负责人决定不跑那次付费证据，本卡就此收口（2026-09-05）。**
>
> **出处**：产品负责人 2026-09-05，本会话，原话 ——
> 「TASK-041 的一次真实付费证据 —— 约 $0.28（40 日元）跑一条 MiniMax 视频。…
> 这个以前就跑过了。不要浪费钱了收了吧。」
>
> **收口的口径必须写清楚，否则这张 done 卡会被读成「付费路已经证过了」：**
>
> - 增量 1 / 2 / 4（核心命令、UI 接线、证据运行前置）**是有代码级证据的完成**，
>   守卫测试俱在，见下方那张表；
> - 增量 3（1 次真实证据）**是「决定不做」，不是「已验证」**。收口当天我在
>   仓库与他的真实项目根（`MotvProjects`）里逐处找过那次真跑会留下的痕迹 ——
>   **预留记录（`budget/reservations/`）、QCD 事件、付费台账、staging 产物，四处皆空**。
>   所以「以前跑过」这句话在仓库里没有对应物；它可能发生在别处或别的时期，
>   但**本仓库没有它的证据**。
>
> **因此仍然为真的一句话**：MiniMax 付费视频这条路**从未在这个仓库里被真实跑通过**。
> 谁要再依赖它，得自己重新拿证据 —— 这不是悲观，这是这张卡收口时的事实。
>
> **不影响的**：付费闸、窄授权（ADR-0006 / ADR-0009 / ADR-0041）、离线守卫，
> 一个字都没动。要跑的那天，命令与前置都在这张卡里写着。
...
### 这张卡还带着 TASK-136 一起走（2026-09-05 登记）

本卡的增量 2（Studio 侧付费命令接线）会**第一次**在 `runstore` 里造出
`executor = "provider:*"` 的 Run —— 今天整个 Studio 里一个都没有。
[ADR-0099](../../adr/ADR-0099-resume-is-not-a-retry.md) 决策 8 因此把
[TASK-136](../backlog/TASK-136-generation-resume-and-idempotency.md)（重启后的续跑与幂等）
钉在这张卡上：**付费写入方落地的那个提交，同时接上 `operationRef` 与续跑路径**。

不这么排的代价是具体的：那一刻起，任何一次重启（改代码、部署、崩溃）都会留下一条
`failed` + `sideEffect: "unknown"` 的记录，而按今天 `runstore.create()` 的去重规则，
用户唯一走得通的出口是**显式重试 = 重新提交 = 第二次扣费**（推演见 ADR-0099 背景）。
先接是一份文档的成本，后接的第一次代价是一笔真钱。
