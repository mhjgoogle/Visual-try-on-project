...
- 状态：**已完成（2026-08-24 收口）。** 批次一整批交付（`70dab40`）；
  批次二 / 三的「未完成」清单里**已无真正的欠账** —— 剩下的两项（`persist.js`
  改经 apiclient、`runtime.js` 的 `cancelRun`/`runOnExecutor`）卡上写着
  **「未做，有意保留」并给了理由**，那是**已记录的非目标**，不是漏做；
  验收 #5 的守卫已于 2026-08-24 补齐（`tests/backend500.test.mjs`）；
  独立审查早在 2026-08-16 补完（见 §7.3 订正）。
  ~~批次二 / 三部分完成（2026-08-14/15）~~
...
- 前置：**已满足**
  ...
  - **TASK-064 / 065 / 066 / 067 / 069 / 070 / 071 已验收**（产品负责人 2026-08-13
    随 ADR-0066 批准一并收口）；
  - **[TASK-068](../done/TASK-068-legacy-agent-endpoints-to-runtime.md) 并入本卡 §1.8，
...
[TASK-068](../done/TASK-068-legacy-agent-endpoints-to-runtime.md) **不单独排期**，其内容并入本项：
...
[TASK-064](../done/TASK-064-creator-ui-consolidation.md) §4d–§4m 按 AGENTS.md 第 17 条转记了
...
**~~未做 Codex 独立审查~~ → 已补审（2026-08-16）。** 上面那句写于 2026-08-15，
比补审早一天，2026-08-24 复查为过期：[待复审清单](../../design/active/pending-codex-rereview.md)
里「工作树那一大批」经批次 A / I 补审 —— 报 3 条，2 条驳回（输入面决定围栏范围；
`replacesKey` 在 dispatcher 第一行即校验）、1 条「顶层 `reviews` 无 schema 校验」
已于 `cb038ac` 修掉。该清单的「仍然待办」一节现在只剩一条与本卡无关的 POSIX 记录。
