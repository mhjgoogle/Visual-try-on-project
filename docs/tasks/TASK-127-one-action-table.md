...
- 状态：完成 · **实现完成**（2026-09-05；证据见 §实施记录）。**真浏览器已走过一遍**：[TASK-130](TASK-130-connected-sample-and-journey.md) 的旅程在可重复样本上打字进故事核心（`uiAct` → `work.core`），刷新后字还在；**真实用户项目**上的人工走查仍归产品负责人本人（收敛审查 §9）。ADR-0096 决策 4（派生页面地图 / 能力输入）不在本卡，另开卡
...
- 剧集制作侧（`epprod.js` / `postconsole.js` 的 `act()` dispatch 走的是 `ctx.actions.dispatch`
  另一套 envelope）→ [TASK-128](TASK-128-episode-side-actions-into-the-table.md)。
- 作品设定的**结构**写（人物 / 场景地的状态与参考图、关系的增删与方向、节拍、改名、软删）
  → [TASK-129](TASK-129-settings-structure-writes-into-the-table.md)。合同里它们是
...
| 第三块面 | `workspaces.js#bindSettings` 还有 18 种结构写（状态 / 参考图 / 改名 / 软删 / 声音 / 覆盖），`relws` 3 种，`epplanws` 节拍 4 种 | **不塞进本卡**：那是带实体身份的增删，进表前要逐条核可逆性 → [TASK-129](TASK-129-settings-structure-writes-into-the-table.md)。合同里做成**棘轮**：`DEFERRED_BINDERS` 钉死这 25 个名字，只能减少不能增加，且每个名字必须原样出现在 TASK-129 上（两条测试互为反向：新名字进不来、接好的必须划掉） |
