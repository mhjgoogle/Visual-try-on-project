...
- 状态：完成 · **已完成（2026-08-23 复查收口，ADR-0082）**
  —— ~~进行中~~ 是状态漂移。证据：canvas 迁移 `v11 → v12` 在位；`/api/runtimes` 在 `server.py` 与 `src/services/runtime.js` 两侧都在（验收项 1）。Skill 包此后由 ADR-0067 / [TASK-075](TASK-075-product-skill-package.md) 进一步承接。
