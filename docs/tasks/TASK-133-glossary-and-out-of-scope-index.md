...
- 技术目标：本仓库**当前事实找不到，于是被重新发明** —— 这是文档漂移的机理，也是
  ...
  1. **术语没有查询入口**。仓库里已散着 80+ 条辨析（「SkillRun 不是第二份记录」
     ...
     97 份 ADR 正文里。[TASK-128](TASK-128-episode-side-actions-into-the-table.md)
...
- 三项重架构借鉴只**开卡记账不实施**：[TASK-134](TASK-134-import-linter-layering-contract.md)
  （import-linter 分层契约）· [TASK-135](TASK-135-server-authoritative-workflow-plan.md)
  （服务端权威 next_action）· [TASK-136](TASK-136-generation-resume-and-idempotency.md)
  （生成任务重启续跑与幂等）。
