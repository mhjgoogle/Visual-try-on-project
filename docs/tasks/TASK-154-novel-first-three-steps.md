...
> **2026-09-18 真实运行时走查（第 2 条已闭合，第 1 条部分闭合）。**
> `structure-planner` 的真实提示词跑在真 claude 上（67s），答案过 schema **PASS** →
> `proposePlanRows` → `applyPlanProposal` **六行九列全部填上**、`悬空引用 0`。
> 九列不是敷衍：`冲突`「电子档与纸质归档件检索时自动跳过，值班的许渡按规定要清空这一层」、
> `关键转折`「在待销毁物底部摸到不属于归档体系的簿子，同时发现自己想不起妹妹脸上的痣在哪侧」。
>
> **`outlineRefs` 的编号锚点实测两种形状**：作品里写了大纲（应用的正常路径，`outline` =
> 全部段落 join 空行）时，模型**按段落数得对**（EP1→[1,2]、EP2→[3]…，它自己的 notes 也写
> 「由大纲第 3、4 段推演」）；而**旧版「已批准大纲」回落**的那种结构化对象里没有段落，模型
> 会自行编号（实测按顶层字段 1=storyCore…6=themeAndChange）。后者**不会静默错写** ——
> 那条回落只在 `work.outline.nodes` 为空时发生，于是每个引用都越界、被丢弃并在回执里报出
> 「N 处大纲引用指向不存在的段落，已丢弃」。结果是九列填上、关联那一列空。
> 记 [TASK-087 §6.21](TASK-087-followup-ledger.md)，不在本卡修。
>
> **仍未被看过**：浏览器里按那一下、整表替换后回收区拿得回来、以及填得好不好。
