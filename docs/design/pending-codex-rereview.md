...
| TASK-097 批次 0（共同机制，见下方提交号） | `batchpay.js` / `refset.js` / `counts.js` / `genspec.js` / `refscan.js` 五个新纯模块 + `gencard.js` 改读 `genspec` | 高（付费批量 + 参考绑定身份 + 跨层合同） | **codex 跨模型 6 轮**（名义预算 3，第 4–6 轮为显式声明的超额）。六轮共报 **15 个 P1 + 3 个 P2，全部为真、全部已修、零驳回**；**第 6 轮那 3 个 P1 的修复未经复审**，按 ADR-0069 决策 d 显式选择 ship | **待 codex 复审**（范围：**只需**第 6 轮那 3 处修复——`refset` 的有效 ordinal 冲突检测、`batchpay.spendComplete` 计入 pending、`settlementLine` 先看状态。前五轮的修复都已被下一轮 codex 看过）。**注**：这两个模块今天零调用方，第一批消费者是本链批次 2 / 4D / 4F / 4G / 4E，每一批高风险 2 轮审查会在真实调用语境里重新审它们 |
