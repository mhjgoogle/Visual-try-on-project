...
| TASK-085 轮 2 的两处 P2 修复未经复审 | **待补审，push / merge 前必须完成**（CLAUDE.md：高风险改动的审查是后移，不是取消）。TASK-085 高风险预算 2 轮已用尽，轮 2 `VERDICT: pass` 无 P1——按 CLAUDE.md「仅当第 2 轮仍报 P1 才允许第 3 轮」，不得再开一轮，因此这两处修复**没有任何审查者看过**：① `_selects_all_tracked` 改为按短选项簇从左往右扫、遇取值选项即停（修 `-Salpha` 被读成带 `-a`）；② `--intent` 改为在主循环内解析以遵守 `--` 分隔符（修暂存文件名 `--intent` 会把分类器调用切成意图模式）。两处都在 `commit_gate_policy.py`，都属**质量门本身**（高风险档）。已各自补测试并做变异验证（18 个变异全杀、0 存活），但变异验证不是独立审查 |
