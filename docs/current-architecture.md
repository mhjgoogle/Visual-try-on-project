...
| **执行器分两条路** | **创作能力**：`claude-code`，`--tools ""` 工具全关、中性临时目录 —— 它的提示词内嵌作品正文，那是注入面。**开发指令**：`claude-dev`，带工具、在仓库根跑、能留下提交 —— 它的提示词**一个字作品内容都不带**，这是它敢带工具的全部理由。两个执行器条目**并存**，不是一个带开关的条目；能写仓库的那个只对 `dev.implement` 开放（`_REPO_WRITING_TASK_TYPES` 白名单，闸落在 `_execute_run`），只能从「开发」窗口发起。**分支边界靠核对不靠禁止**：它握着写权限，能 checkout、能改钩子 —— 所以起跑前记快照、落地时比对分支 / 本地 main / 远端 main（`git ls-remote`，不是本地跟踪 ref），越界或**读不到**都逐条写进他看得到的回执。硬隔离（一会话一 worktree）是 TASK-143，尚未做 | [ADR-0056](adr/ADR-0056-local-ai-runtime-and-film-skills.md) 决策 2 · [ADR-0107](adr/ADR-0107-development-instructions-run-with-tools-in-the-repo.md) · [TASK-160](tasks/TASK-160-your-word-drives-the-build.md) |
