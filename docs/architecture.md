...
第一阶段这些概念以结构化文件（JSON/YAML）表达，阶段 1 才落为 Python 数据模型。
...
- 发布任务：为每个 Shot 生成一份**人工制作任务说明**（Markdown/JSON）的内容，
  ...
  （由编排器分配的 inbox/staging 位置）；任务说明内容作为结构化结果返回，
  由编排器落盘，并返回 `waiting_for_user` 状态；
- 用户在网页视频工具中手工生成视频，并按任务说明把文件放入指定的
  inbox/staging 位置；
- 检查进展：检查该位置下文件是否出现，出现后返回 `asset_available` 状态；
- 收集产物：**只返回**用户放入 inbox/staging 的文件引用（结构化结果）——
  由编排器对文件做校验（存在、命名、容器格式、可解码、基本参数），通过后
  将媒体导入正式资产目录、登记 VideoAsset、将任务置为 done；校验不通过时
  由编排器标记任务失败并输出明确的错误报告。
...
- 一切中间产物（剧本、分镜数据、任务说明、视频、校验报告、合成结果、QCD 记录）
  都以**文件**形式保存在项目目录内，人类可读优先（JSON/YAML/Markdown/MP4）；
