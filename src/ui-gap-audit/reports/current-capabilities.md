...
AI 剧本拆解 → 人物/场景地提案 → 逐条确认进「作品设定」
  ↓
AI 生成分镜草稿（38 镜）→ 手工编辑 → 「锁定为正式版本」【唯一写核心文件】
  ↓
为镜头绑定参考资产（从资产库或上传）
  ↓
...
—— `照见未明rev2` 里有 48 集规划、6 个角色档案、38 个分镜、9 个资产，全是真的。
...
| 审片 | 工作区▾ › 审片 | C-016 | Full | Confusing | `evaluation_*` / `action_center` | 38 个分页码平铺；通过/跳过没有去处 |
...
| 场景层 | 领域模型完整，真实项目里 **48 集全部 0 场景 0 归属**，38 个镜头挂在「未分配到场景」（C-010） |
...
| D-6 | 核心 `/shots` 返回 `[]`，canvas 里有 38 个镜头 | 实测 | 创作领域与核心领域**没有同步**，直到「锁定分镜」那一刻才写核心 |
