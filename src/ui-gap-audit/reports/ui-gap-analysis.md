...
canvas 里的 `38 镜 / 0 已生成`）。只是没渲染。

**Required change** 卡片 = 首个可用参考图或成片首帧 + 「48 集 · 38 镜 · 0 已生成」+
...
现在的进度只有各页各自的局部计数（`0/38`、`38 镜`），没有全局。
...
38 个镜头全在「未分配到场景」。C-012 参考统筹 0 绑定。
...
| 3c | **分镜是卡片不是表**（38 镜的景别/运镜/情绪全「未记录」）；**画面描述里的实体没链接 → 资产缺口算不出来** | GAP-24 / GAP-25 | **P1** | MODIFY / ADD | 无 / 小改 |
| 4 | **一次生成被拆在四个地方，价格藏在最后** —— 真实项目 0/38 卡在这里 | GAP-15 | **P1** | COMBINE | 无 |
| 5 | **没有「整集拍平成一张可审阅清单」的视图** —— 审片是 38 个页码逐个翻 | GAP-22 | **P1** | ADD | 无 |
...
| 10 | 运镜/风格/特效自由文本 38 镜全空；模型对用户完全不可见；AI 导演每页一个样 | GAP-16 / GAP-20 / GAP-08 | **P2** | ADD / EXPOSE / COMBINE | 部分需 ADR |
