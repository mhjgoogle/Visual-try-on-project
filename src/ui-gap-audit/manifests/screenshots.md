...
## Current — 第五轮（**2026-08-20，TASK-097 剧集制作链收口后**）

**捕获条件**：Playwright Chromium · viewport 1600×1000~1200 · 真实项目
`照见未明rev2` · `--port 8770` · **JS 异常 0** · **全程未发生任何真实扣费**。

**这一轮项目状态**：60 个镜头（AI 拆的），**0 视频 / 0 音频 / 0 生成记录 / 9 张图片**。
这不是缺陷 —— 它恰好是这一链最需要证明的那件事的实测环境：**「不知道」不许印成「0」，
也不许印成「没做」**。下面每一张的重点都在这里。

| ID | Screenshot | 它证明了什么 |
| --- | --- | --- |
| C-130 | `07f-production-wizard.png` | **五步向导**：任何时刻只有一个下一步；顶部数字全部来自登记表（`60 个镜头已就绪` / `1/2 已生成 · 差 1 个` / `60/60 已合成`） |
| C-131 | `07g-shot-table-scenes.png` | 分镜表**按场景分组** + 删除是**可撤销**的（软删除 + 回收区） |
| C-132 | `07h-asset-prep.png` | 第 ② 步三组卡片 + 全局风格；**「一个都没识别出来」与「都已经有设定图」是两种不同的显示** |
| C-133 | `07i-prompt-batch.png` | ③ 一键合成全部提示词：本地编译，`spent: 0` 是「**确知**没花」而不是「不知道」 |
| C-134 | `07j-storyboard-strip.png` | ④ 那条横向带：`0/60 已通过 · 60 还没画`；跳过与「还没画」在屏幕上分得开 |
| C-135 | `07k-keyframe-compose.png` | ⑤ 全集清单：闸门**全部关着且逐行说明原因**，主行动降级成「仍然进去看看」 |
| C-136 | `07l-video-batch.png` | ⑥ 批量生视频：`0/60 视频已完成`；点下去如实说「60 镜还没有首帧」；**拿不到总额时说原因 + 给替代路径，不伪造金额** |
| C-137 | `10a-post-status.png` | **后期状态条**（5A）：三行如实说「60 镜要不要做还没写下来」/「60 镜还不能开始」，声音资产在这里落位并写明为什么不在第 ② 步 |
| C-138 | `10b-post-status-parallel.png` | **本链最要紧的一张**：给第 1 镜写上一句台词，立刻出现「**1 处现在就能开始 —— 不用等视频；定稿再等画面**」并说明定稿卡在哪。产品负责人那句「音频在视频之后，但可以并行准备」第一次在界面上成立。（验证造成的台词改动已在同一次运行里改回） |
| C-139 | `10c-wizard-counts.png` | 向导 ④/⑤ 的计数**真的会动** —— 跳过一镜草图后出现「· 1 镜跳过」。修之前那三个计数读的是生产 board 上不存在的键，**永久是 0 而 known 为真** |
| C-140 | `10d-shot-qc.png` | **逐镜质检**（5B）：一致性说的是「没有绑定任何设定图 —— 没有可核对的东西」，**不是「没用上」**；表列 20 行并写明「另有 40 镜同样有发现，未列出」 |
